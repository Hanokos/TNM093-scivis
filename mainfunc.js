...
// For all shaders, "#version 300 es" has to be the first token in the source, so
//
// var vertexSource = `
// #version 300 es
//
    ...
    while (t < tEnd && result.a < 0.99) {
      ...
      if (color.a > 0.0) {
        if (compositingMethod == CompositingMethodFrontToBack) {
          ...
          // result is vec 4 with the accumulative values
          // color is current value
          // (1-result.a) från slide 124 lecture SCiviss
          ...
          //från slide 122
          // change 0.1 ? donno which intensity threshold
          if(color.a > 0.25){
            result = color;
            break;
          }

          //intensity is the scalar value of the scalar field of the density in this case. 
          //Contour lines on a map marking areas of equal elevation. ^^
          ...
          // slide 123
          // change result if current a is larger
          if(result.a < color.a){
            result = color;
          }
...
function updateTransferFunction(gl, transferFunction) {
  ...
  // Hämta noddata, som sannolikt innehåller information om olika grafiska noder.
// Hämta noddata, som sannolikt innehåller information om olika grafiska noder.
// Varje nod kan ha egenskaper som färg, opacitet, och start/slutpositioner.
var nodeData = getNodeData(); 

// Skapa en array som ska hålla genomsnittlig färgdata för 256 pixlar.
// Varje pixel har 4 kanaler: Röd, Grön, Blå och Alfa (RGBA), så arrayen får en storlek av 256 * 4.
let averageData = Array.from(Array(256 * 4)).fill(0); 

// Skapa en array för att räkna antalet noder som påverkar varje pixel.
// Denna array har 256 element, en för varje pixel.
let numNodesAt = Array.from(Array(256)).fill(0);

// Loopar igenom varje nod i nodeData-arrayen för att bearbeta dess egenskaper.
for (let i = 0; i < nodeData.length; i++) {
    // Extrahera start- och slutvärden för opacitet från noden.
    let opacityStart = nodeData[i].opacityStart; // Innehåller startpunkten och värdet för opaciteten
    let opacityEnd = nodeData[i].opacityEnd;     // Innehåller slutpunkten och värdet för opaciteten
    let start = nodeData[i].start;               // Startposition för noden (kan vara en pixelindex)
    let end = nodeData[i].end;                   // Slutposition för noden
    let color = nodeData[i].color;               // Färgvärden (RGBA) i en array

    // Loopar genom intervallet från start till slutposition.
// Multiplicerat med 4 för att justera indexet för RGBA-värdena.
    for (let j = start * 4; j < end * 4; j += 4) {
        // Hämta den opacitet som gäller för den aktuella punkten (pixeln).
        let opacity = getOpacityAtPoint(j, opacityStart, opacityEnd, start, end); 
        
        // får färgvärden gånger  opaciteten för den röda kanalen.
        averageData[j] += color[0] * opacity;   // Red channel: multiplicera färgvärdet med opaciteten
        // får färgvärden gånger  opaciteten för den gröna kanalen.
        averageData[j + 1] += color[1] * opacity; // Green channel
        // får färgvärden gånger  opaciteten för den blå kanalen.
        averageData[j + 2] += color[2] * opacity; // Blue channel
        
        // får opaciteten  alfa kanalen.
// Om summan av den aktuella opaciteten och den ackumulerade överstiger 255, sätt värdet till 255.
        (averageData[j + 3] + opacity) >= 255 ? averageData[j + 3] = 255 : averageData[j + 3] += opacity;
        
        // Öka antalet noder som påverkar den aktuella pixeln.
// Genom att dela med 4 får vi rätt index för pixeln i numNodesAt-arrayen.
        numNodesAt[j / 4] += 1; 
    }
}

// Loopar igenom averageData för att beräkna de genomsnittliga färgerna för varje pixel.
for (let i = 0; i < averageData.length; i += 4) {
    // Normalisera färgkanalerna genom att dela med det ackumulerade alfavärdet.
   // Detta undviker division med noll genom att kontrollera att alfakanalen är olika noll.
    averageData[i + 3] != 0 ? averageData[i] /= averageData[i + 3] : averageData[i] = 0; // Röd
    averageData[i + 3] != 0 ? averageData[i + 1] /= averageData[i + 3] : averageData[i + 1] = 0; // Grön
    averageData[i + 3] != 0 ? averageData[i + 2] /= averageData[i + 3] : averageData[i + 2] = 0; // Blå
}


// Loopar igenom averageData igen för att tillämpa en skalningsfaktor baserat på antalet noder som påverkar varje pixel.
for (let i = 0; i < averageData.length; i += 4) {
    // Beräkna en skalningsfaktor som varierar beroende på antalet noder som påverkar den aktuella pixeln.
// Ju fler noder som påverkar, desto lägre faktor, vilket kan minska intensiteten.
    let factor = 1.15 - 0.15 * numNodesAt[i / 4]; 
    
    // Uppdatera data-arrayen med de nya värdena, se till att de inte överskrider 255.
// Detta säkerställer att färgerna hålls inom RGB:s giltiga interval.
    data[i] = Math.min(averageData[i] * factor, 255); // Uppdatera röd kanal
    data[i + 1] = Math.min(averageData[i + 1] * factor, 255); // Uppdatera grön kanal
    data[i + 2] = Math.min(averageData[i + 2] * factor, 255); // Uppdatera blå kanal
    data[i + 3] = averageData[i + 3]; // Sätt alfakanalen, som inte påverkas av faktorn
}

// Funktion för att beräkna opacitet vid en given punkt.
// Denna funktion används för att interpolera opacitetsvärden baserat på nodens position.
function getOpacityAtPoint(i, opacityStart, opacityEnd, start, end) {
    // Beräkna interpoleringspunkter för att avgöra hur opaciteten ska förändras.
    let point1 = (start - start) / (end - start); // Punkt 1, alltid 0
    let point2 = (opacityStart[0] - start) / (end - start); // Punkt 2 baserat på opacityStart
    let point3 = (opacityEnd[0] - start) / (end - start); // Punkt 3 baserat på opacityEnd
    let point4 = (end - start) / (end - start); // Punkt 4, alltid 1
    
    // Beräkna den normaliserade positionen mellan start och slut.
    let interPolPoint = (i / 4 - start) / (end - start); // Mellan 0 och 1

    // Bestäm opaciteten baserat på den aktuella positionen i förhållande till de definierade punkterna.
    // Om den interpolerade punkten är mellan point1 och point2 beräknas opacitet.
    if (interPolPoint >= point1 && interPolPoint < point2) {
        // Beräkna och returnera en gradvis ökning av opaciteten fram till opacityStart[1].
        return opacityStart[1] * (interPolPoint - point1) / (point2 - point1); 
    } 
    // Om den interpolerade punkten är mellan point2 och point3 beräknas opacitet med en övergång mellan opacityStart och opacityEnd.
    else if (interPolPoint >= point2 && interPolPoint < point3) {
        let scale = (interPolPoint - point2) / (point3 - point2); 
        return opacityStart[1] * (1 - scale) + (opacityEnd[1]) * scale; 
    } 
    // Om den interpolerade punkten är mellan point3 och point4 beräknas opacitet som en gradvis minskning av opacityEnd.
    else if (interPolPoint >= point3 && interPolPoint < point4) {
        let scale = (interPolPoint - point3) / (point4 - point3); 
        return opacityEnd[1] * (1 - scale); 
    } 
    // Om interpolationspunkten ligger utanför intervallet, returnera 0.
    else {
        return 0; 
    }
}
...
// Ladda upp de nya datan till texturen i grafiken.
gl.bindTexture(gl.TEXTURE_2D, transferFunction); // Binda texturen för att göra uppdateringar.
gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 256, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE, data); // Ladda upp datan som en 2D-textur.
